...
uniform float Similarity <
	...
	ui_tooltip = "Extra Image clamping based on Depth Similarities between Past and Current Depth for DeGhosting TAA.\n"
...
float2 Halton23(int index) {
    ...
    for (int i = 0; i < 16; i++) {
        if (n <= 0) break;
        result.x += (n % 2) * invBase2;
        n /= 2;
        invBase2 /= 2.0;
    }
    float invBase3 = 1.0 / 3.0;
    n = index;
    for (int j = 0; j < 16; j++) {
        if (n <= 0) break;
        result.y += (n % 3) * invBase3;
        n /= 3;
        invBase3 /= 3.0;
    }
    return result;
...
{   
    ...
    float3 previousColor = tex2D(PSBackBuffer, texcoord + jitterDeltaUV).xyz;
    float diff = length(BB - previousColor) * lerp(1.0, 8.0, Delta_Power);
    if (Delta == 1)
        diff = V_Buffer;
